...
var demo = new Vue({
  ...
  data: {
    //ListWorkspace
    ListWorkspaceAvailable :[['https://dokumen.bi.go.id/sites/documentCenter_gbioffc','GBI Office',''],['https://dokumen.bi.go.id/sites/documentCenter_DGdoc','Shared Documents','f580e872-ed77-4f76-a074-27797963df6c'],['https://dokumen.bi.go.id/sites/documentCenter_dgsoffc','DGS Office'],['https://dokumen.bi.go.id/sites/documentCenter_adg3offc','ADG Bidang 3 Office'],['https://dokumen.bi.go.id/sites/documentCenter_adg4offc','ADG Bidang 4 Office'],['https://dokumen.bi.go.id/sites/documentCenter_adg5offc','ADG Bidang 5 Office'],['https://dokumen.bi.go.id/sites/documentCenter_adg6offc','ADG Bidang 6 Office']],
    //treeview
    treeData: treeData,
    isclicked: false,
    ...
    testData:{"workspace": "1,2,3,4,5,","GBI": "1,3,2"},
    //uploadFile
    files: [],
    //forget
    akunUser: '',
  ...
  mounted() {
    ...
    // $.ajax({
    //   url: "https://dokumen.bi.go.id/sites/Workspace_Dev/_api/search/query?querytext='contentclass%3aSTS_Site+AND+path%3ahttps%3a%2f%2fdokumen.bi.go.id%2fsites%2f+AND+documentCenter'&trimduplicates=false&rowlimit=10&selectproperties='Event%2cKeyword'&sortlist='Rank%3adescending'",
    //   type: "GET",
    //   context: this,
    ...
    //                 this.treeviewCopy.children.push({name: data.d.Name, level: 1, filesUri: data.d.Files.__deferred.uri, folderUri: data.d.Folders.__deferred.uri, folderLocation: myArr[0] + "/" + myArr[1] + "/" + myArr[2] + data.d.ServerRelativeUrl, metadata: data.d.ListItemAllFields})
    ...
    //       }
    //       // this.listPersonal.push(data.d)
    //     // this.listParent = data.d.Folders.result
    //   }
    // });
    // $.ajax({
    //   url: "https://dokumen.bi.go.id/sites/mysite/personal/" + temp[1].replace('.', '_') + "/_api/Web/GetFolderByServerRelativeUrl('/sites/mysite/personal/" + temp[1].replace('.', '_') + "/Personal Lib/')?$expand=Folders/ListItemAllFields,Files/ListItemAllFields,Files/Author,ListItemAllFields/File,Activities,Files/ListItemAllFields/Properties",
    //   type: "GET",
    //   context: this,
    //   headers: { "accept": "application/json;odata=verbose" },
    //   success: function onSuccess(data) {
    //     const dataResponse = data.d.Folders.results
    //     console.log('response')
    //     console.log(dataResponse)
    //     this.personalUrl = temp[1].replace('.', '_')
    //     this.listPersonal = data.d.Folders.results
    //     this.sorting(this.listPersonal)
    //     for (i in data.d.Folders.results){
    //       if(data.d.Folders.results[i].Name != 'Forms'){
    //       this.treeviewCopy.children.push({name: data.d.Folders.results[i].Name, level: 1, filesUri: data.d.Folders.results[i].Files.__deferred.uri, folderUri: data.d.Folders.results[i].Folders.__deferred.uri, folderLocation: myArr[0] + "/" + myArr[1] + "/" + myArr[2] + data.d.Folders.results[i].ServerRelativeUrl, metadata: data.d.Folders.results[i].ListItemAllFields})
    //       }
    //     }
    ...
    //   error: function (ex){
    //     console.log(ex)
    //     console.log(this.listPersonal.length)
    //     this.isPersonalLoading = false
    //   }
    // });
  ...
  methods: {
    sortWorkspace(status){
        this.listParent = _.orderBy(this.listParent, 'name', status)
    ...
    clickedBoss: function (parent) {
      ...
      this.isFolderClicked = false;
        ...
        alert('anda tidak punya aksesss')
      }
    },
    ...
    getParent: function (parent, location) {
      urlAsli = parent.Folders.__deferred.uri + "?$expand=ListItemAllFields"
      ...
      if (!!this.searchField.trim()) {
        ...
        $.ajax({
          ...
          success: function onSuccess(data) {
            ...
            console.log(this.yangDimunculin)
          }
        });
      } else
        alert('search field kosong')
    ...
    openModalCopyFile(parent, item) {
      this.isCopyModalClicked = true
      if (item == "") {
      ...
      // this.isParentClicked = true;
      // this.listFile=[];
      // this.getParent(parent, 'personal');
      // this.getChild(parent, 'personalFile');
      // this.onFolderSearch = false
      // this.isFolderClicked = false;
    },
    ...
    bukaModalCopyVue: function (ext) {
            ...
            this.openModalCopyFile(this.listPersonal[1],"drag")
    ...
    var itemProperties = {'IdUser':_spPageContextInfo.userId,'IdFolder': this.selectedIdParent,'Message':'ini test'};
//create item
    createListItem(_spPageContextInfo.webAbsoluteUrl,'Access Request',itemProperties,
   function(entity){
   ...
   function(error){
    ...
    testApi: async function () {
      ...
      var content = JSON.stringify(this.testData);
// var digest = $("#__REQUESTDIGEST").val();
// $.ajax({
//         url: "https://dokumen.bi.go.id/sites/Workspace_Dev/WebPart/_api/contextinfo",
//         type: "POST",
//         headers: {
//           "Accept": "application/json;odata=verbose"
//         },
//         success: function (data) {
//             let digest = data.d.GetContextWebInformation.FormDigestValue
//           $.ajax({
                       
//                       //  url: "https://dokumen.bi.go.id/sites/documentCenter_DGdoc/_api/Web/Lists(guid'186be5f1-c28b-4a66-bf65-1bf92bd3d439')/Items",
//                       //Document Library = 2695da18-3118-48aa-b59b-bb8b2148992e

//                 url: "https://dokumen.bi.go.id/sites/documentCenter_DGdoc/_api/web/lists/getbytitle('Document Library')/items?$expand=FieldValuesAsText",
//                 // url: _spPageContextInfo.webAbsoluteUrl + "/_api/Web/Lists(guid'520a630a-de40-496c-9d1b-a21a4110b35a')/Items",
//                 type: "POST",
//                 context: this,
...
// })
$.ajax({
                url: "https://dokumen.bi.go.id/sites/Workspace_Dev/_api/web/lists/getbytitle('Access Request')/items?$filter=(IdUser eq 11) and (IdFolder eq 699) ",
                type: "GET",
                context: this,
                headers: { "accept": "application/json;odata=verbose" },
                success: function onSuccess(data) {
